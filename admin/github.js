...
async function _getIssues(user, password) {
    ...
    let issues = await fetchGitHub(user, password, "https:/\/api.github.com/repos/420integrated/fourtwentycoins.js/issues?state=all&per_page=100", cacheOnly)
...
async function createRelease(user, password, tagName, title, body, prerelease, commit) {
    ...
    const result = await fetchJson({
        url: "https://api.github.com/repos/420integrated/fourtwentycoins.js/releases",
        user: user,
        password: password,
        headers: headers
