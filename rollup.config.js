...
function getConfig(minify, buildModule, testing) {
    let input = "packages/fourtwentycoins/lib/index.js"
    let output = [ "umd" ];
    let format = "umd";
    let mainFields = [ "browser", "main" ];

    if (buildModule) {
        input = "packages/fourtwentycoins/lib.esm/index.js";
        output = [ "esm" ];
        format = "esm";
    ...
    const outputFile = [
        "packages",
        (testing ? "tests": "fourtwentycoins"),
        ("/dist/fourtwentycoins." + output.join(".") + ".js")
    ].join("/");
