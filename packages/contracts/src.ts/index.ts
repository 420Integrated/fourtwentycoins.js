...
import { checkResultErrors, EventFragment, Fragment, FunctionFragment, Indexed, Interface, JsonFragment, LogDescription, ParamType, Result } from "@420integrated/abi";
import { Block, BlockTag, Filter, FilterByBlockHash, Listener, Log, Provider, TransactionReceipt, TransactionRequest, TransactionResponse } from "@420integrated/abstract-provider";
import { Signer, VoidSigner } from "@420integrated/abstract-signer";
import { getAddress, getContractAddress } from "@420integrated/address";
import { BigNumber, BigNumberish } from "@420integrated/bignumber";
import { arrayify, BytesLike, concat, hexlify, isBytes, isHexString } from "@420integrated/bytes";
import { Deferrable, defineReadOnly, deepCopy, getStatic, resolveProperties, shallowCopy } from "@420integrated/properties";
...
export interface Overrides {
    smokeLimit?: BigNumberish | Promise<BigNumberish>;
    smokePrice?: BigNumberish | Promise<BigNumberish>;
...
async function populateTransaction(contract: Contract, fragment: FunctionFragment, args: Array<any>): Promise<PopulatedTransaction> {
    ...
    if (ro.smokeLimit != null) { tx.smokeLimit = BigNumber.from(ro.smokeLimit); }
    if (ro.smokePrice != null) { tx.smokePrice = BigNumber.from(ro.smokePrice); }
...
function buildEstimate(contract: Contract, fragment: FunctionFragment): ContractFunction<BigNumber> {
    ...
    return async function(...args: Array<any>): Promise<BigNumber> {
        ...
        return await signerOrProvider.estimateSmoke(tx);
    };
}
...
export class Contract {
    ...
    readonly estimateSmoke: { [ name: string ]: ContractFunction<BigNumber> };
