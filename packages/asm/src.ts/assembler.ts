...
function hexConcat(values: Array<Opcode | fourtwentycoins.utils.BytesLike>): string {
    return fourtwentycoins.utils.hexlify(fourtwentycoins.utils.concat(values.map((v) => {
        if (v instanceof Opcode) { return [ v.value ]; }
        if (typeof(v) === "number") {
            if (v >= 0 && v <= 255 && !(v % 1)) {
                return fourtwentycoins.utils.hexlify(v);
            } else {
...
class Script {
    ...
    constructor(filename: string, callback: (name: string, context: any) => any) {
        fourtwentycoins.utils.defineReadOnly(this, "filename", filename);

        fourtwentycoins.utils.defineReadOnly(this, "contextObject", this._baseContext(callback));
        fourtwentycoins.utils.defineReadOnly(this, "context", vm.createContext(this.contextObject));
    }
    ...
    _baseContext(callback: (name: string, context: any) => any): any {
         return new Proxy({
            ...
            fourtwentycoins : fourtwentycoins ,
            utils: fourtwentycoins.utils,

            BigNumber: fourtwentycoins.BigNumber,

            arrayify: fourtwentycoins.utils.arrayify,
            concat: hexConcat,
            hexlify: fourtwentycoins.utils.hexlify,
            zeroPad: function(value: fourtwentycoins.utils.BytesLike, length: number) {
                return fourtwentycoins.utils.hexlify(fourtwentycoins.utils.zeroPad(value, length));
            },

            id: fourtwentycoins.utils.id,
            keccak256: fourtwentycoins.utils.keccak256,
            namehash: fourtwentycoins.utils.namehash,
            sha256: fourtwentycoins.utils.sha256,

            parseFourtwentycoin: fourtwentycoins.utils.parseFourtwentycoin,
            formatFourtwentycoin: fourtwentycoins.utils.formatFourtwentycoin,
            parseUnits: fourtwentycoins.utils.parseUnits,
            formatUnits: fourtwentycoins.utils.formatUnits,

            randomBytes: function(length: number): string {
                return fourtwentycoins.utils.hexlify(fourtwentycoins.utils.randomBytes(length));
            },

            toUtf8Bytes: fourtwentycoins.utils.toUtf8Bytes,
            toUtf8String: fourtwentycoins.utils.toUtf8String,
            formatBytes32String: fourtwentycoins.utils.formatBytes32String,
            parseBytes32String: fourtwentycoins.utils.parseBytes32String,

            Opcode: Opcode,

            sighash: function(signature: string): string {
               return fourtwentycoins.utils.id(fourtwentycoins.utils.FunctionFragment.from(signature).format()).substring(0, 10);
            },
            topichash: function(signature: string): string {
               return fourtwentycoins.utils.id(fourtwentycoins.utils.EventFragment.from(signature).format());
            },
...
export abstract class Node {
    ...
    constructor(guard: any, location: Location, options: { [ key: string ]: any }) {
        ...
        fourtwentycoins.utils.defineReadOnly(this, "location", Object.freeze(location));

        fourtwentycoins.utils.defineReadOnly(this, "tag", `node-${ nextTag++ }-${ this.constructor.name }`);

        for (const key in options) {
            fourtwentycoins.utils.defineReadOnly<any, any>(this, key, options[key]);
        }
    }
...
export abstract class ValueNode extends Node {
    ...
    getPushLiteral(value: fourtwentycoins.utils.BytesLike | fourtwentycoins.utils.Hexable | number) {
        // Convert value into a hexstring
        const hex = fourtwentycoins.utils.hexlify(value);
        ...
        const length = fourtwentycoins.utils.hexDataLength(hex);
    ...
    async assemble(assembler: Assembler, visit: AssembleVisitFunc): Promise<void> {
        ...
        if (this.verbatim) {
            if (this.value.substring(0, 2) === "0x") {
                ...
                visit(this, fourtwentycoins.BigNumber.from(this.value).toHexString());
            }
        } else {
            visit(this, this.getPushLiteral(fourtwentycoins.BigNumber.from(this.value)));
        }
        assembler.end(this);
    }
        ...
        if (isOffset && assembler.positionIndependentCode) {
            ...
            if (here > value) {
                ...
                for (let w = 1; w <= 5; w++) {
                    ...
                    if (fourtwentycoins.utils.hexDataLength(literal) <= w) {
                        literal = fourtwentycoins.utils.hexZeroPad(literal, w);
                        break;
                    }
                }
...
export class OpcodeNode extends ValueNode {
    ...
    async assemble(assembler: Assembler, visit: AssembleVisitFunc): Promise<void> {
        ...
        // Append this opcode
        visit(this, fourtwentycoins.utils.hexlify(this.opcode.value));
...
export abstract class LabelledNode extends Node {
    ...
    constructor(guard: any, location: Location, name: string, values?: { [ key: string ]: any }) {
        ...
        values = fourtwentycoins.utils.shallowCopy(values || { });
        values.name = name;
...
export class LabelNode extends LabelledNode {
    async assemble(assembler: Assembler, visit: AssembleVisitFunc): Promise<void> {
        assembler.start(this);
        visit(this, fourtwentycoins.utils.hexlify(Opcode.from("JUMPDEST").value));
        assembler.end(this);
    }
...
export class PaddingNode extends ValueNode {
    ...
    async assemble(assembler: Assembler, visit: AssembleVisitFunc): Promise<void> {
        ...
        padding.fill(0);
        visit(this, fourtwentycoins.utils.hexlify(padding));
...
export class DataNode extends LabelledNode {
    ...
    constructor(guard: any, location: Location, name: string, data: string) {
        ...
        fourtwentycoins.utils.defineReadOnly(this, "padding", new PaddingNode(Guard, this.location));
    }
    ...
    async assemble(assembler: Assembler, visit: AssembleVisitFunc): Promise<void> {
        ...
        const bytecode = fourtwentycoins.utils.concat(this.data.map((d) => assembler.getBytecode(d)));
    ...
    async assemble(assembler: Assembler, visit: AssembleVisitFunc): Promise<void> {
        ...
        if (this.verbatim) {
            if (typeof(result) === "number") {
                visit(this, fourtwentycoins.BigNumber.from(result).toHexString());
            } else {
                visit(this, fourtwentycoins.utils.hexlify(result));
            }
        } else {
...
export function disassemble(bytecode: string): Bytecode {
    ...
    const bytes = fourtwentycoins.utils.arrayify(bytecode, { allowMissingPrefix: true });
    ...
    while (i < bytes.length) {
        ...
        if (!opcode) {
            opcode = new Opcode(`unknown (${ fourtwentycoins.utils.hexlify(bytes[i]) })`, bytes[i], 0, 0);
        ...
        if (push) {
            const data = fourtwentycoins.utils.hexlify(bytes.slice(i, i + push));
            if (fourtwentycoins.utils.hexDataLength(data) === push) {
                op.pushValue = data;
                op.length += push;
                i += push;
            } else {
    ...
    (<Bytecode>ops).getBytes = function(offset: number, length: number): Uint8Array {
        ...
        return fourtwentycoins.utils.arrayify(result);
    };
...
export function formatBytecode(bytecode: Array<Operation>): string {
    ...
    bytecode.forEach((op) => {
        ...
        let offset = fourtwentycoins.utils.hexZeroPad(fourtwentycoins.utils.hexlify(op.offset), 2);
...
class Assembler {
    ...
    constructor(root: Node, positionIndependentCode?: boolean) {
        fourtwentycoins.utils.defineReadOnly(this, "root", root);
        fourtwentycoins.utils.defineReadOnly(this, "positionIndependentCode", !!positionIndependentCode);
            ...
            if (node instanceof LabelledNode) {
                ...
                if (labels[node.name]) {
                    logger.throwError(
                        ...
                        fourtwentycoins.utils.Logger.errors.UNSUPPORTED_OPERATION,
                        { }
                    );
                }
        ...
        fourtwentycoins.utils.defineReadOnly(this, "labels", Object.freeze(labels));
        fourtwentycoins.utils.defineReadOnly(this, "nodes", Object.freeze(nodes));

        fourtwentycoins.utils.defineReadOnly(this, "_parents", Object.freeze(parents));
    }
    ...
    getLinkValue(target: LabelledNode, source: Node): number | DataSource {
        ...
        const bytes = Array.prototype.slice.call(fourtwentycoins.utils.arrayify(info.bytecode));

        fourtwentycoins.utils.defineReadOnly(bytes, "ast", target);
        fourtwentycoins.utils.defineReadOnly(bytes, "source", target.location.source);
        ...
        if (!safeOffset) {
            ...
            fourtwentycoins.utils.defineReadOnly(bytes, "_freeze", function() { });
        }
        ...
        if (safeOffset) {
            ...
            fourtwentycoins.utils.defineReadOnly(bytes, "_freeze", function() {
                 if (frozen) { return; }
                 frozen = true;
                 fourtwentycoins.utils.defineReadOnly(bytes, "offset", bytes.offset);
            });
        }
...
class CodeGenerationAssembler extends Assembler {
    ...
    constructor(root: Node, options: AssemblerOptions) {
        ...
        fourtwentycoins.utils.defineReadOnly(this, "retry", ((options.retry != null) ? options.retry: 512));
        fourtwentycoins.utils.defineReadOnly(this, "filename", resolve(options.filename || "./contract.asm"));
        fourtwentycoins.utils.defineReadOnly(this, "defines", Object.freeze(options.defines || { }));

        fourtwentycoins.utils.defineReadOnly(this, "_stack", [ ]);
        ...
        try {
            if (result.offset < 0) {
                ...
                this._checks.push(() => {
                    ...
                    if (check.offset === result.offset && fourtwentycoins.utils.hexlify(check) === fourtwentycoins.utils.hexlify(result)) {
                        return true;
                    }
                    return false;
                });
            ...
            this._checks.push(() => {
                const check = <DataSource>super.getLinkValue(target, source);
                return (fourtwentycoins.utils.hexlify(check) === fourtwentycoins.utils.hexlify(result));
            });
        }
    ...
    async _assemble(): Promise<void> {
        ...
        await this.root.assemble(this, (node, bytecode) => {
            ...
            offset += fourtwentycoins.utils.hexDataLength(bytecode);
        });
    ...
    async assemble(label?: string): Promise<string> {
        ...
        return logger.throwError(
            ...
            fourtwentycoins.utils.Logger.errors.UNKNOWN_ERROR,
            { }
        );
    }
}
...
export function parse(code: string, options?: ParserOptions): Node {
    ...
    _parser.yy._fourtwentycoins Location = function(loc?: _Location): Location {
    ...
    _parser.yy._fourtwentycoins Location = null;
