...
function equals(actual, expected) {
    ...
    // Uint8Array
    if (expected.buffer) {
        if (!fourtwentycoins_1.fourtwentycoins.utils.isHexString(actual)) {
            return false;
        }
        actual = fourtwentycoins_1.fourtwentycoins.utils.arrayify(actual);
    ...
    // Maybe address?
    try {
        var actualAddress = fourtwentycoins_1.fourtwentycoins.utils.getAddress(actual);
        var expectedAddress = fourtwentycoins_1.fourtwentycoins.utils.getAddress(expected);
...
function getValues(object, named) {
    ...
    switch (object.type) {
        ...
        case 'string':
            return object.value;
        case 'buffer':
            return fourtwentycoins_1.fourtwentycoins.utils.arrayify(object.value);
        case 'tuple':
...
describe('ABI Coder Encoding', function () {
    var coder = fourtwentycoins_1.fourtwentycoins.utils.defaultAbiCoder;
...
describe('Test Contract Events', function () {
    ...
    tests.forEach(function (test, index) {
        it(('decodes event parameters - ' + test.name + ' - ' + test.types), function () {
            this.timeout(120000);
            var iface = new fourtwentycoins_1.fourtwentycoins.utils.Interface(test.interface);
        ...
        it(('decodes event data - ' + test.name + ' - ' + test.types), function () {
            ...
            test.normalizedValues.forEach(function (expected, index) {
                if (test.indexed[index]) {
                    assert_1.default.ok((fourtwentycoins_1.fourtwentycoins.Contract.isIndexed(parsed[index]) && parsed[index].hash == null), 'parsed event data has empty Indexed - ' + index);
                }
                else {
...
describe('Test Interface Signatures', function () {
    ...
    tests.forEach(function (test) {
        it('derives the correct signature - ' + test.name, function () {
            var iface = new fourtwentycoins_1.fourtwentycoins.utils.Interface(test.abi);
            this.timeout(120000);
    ...
    it('derives correct description for human-readable ABI', function () {
        var iface = new fourtwentycoins_1.fourtwentycoins.utils.Interface(["function transfer(address from, uint amount)"]);
        [
            "transfer",
    ...
    // See: https://github.com/420integrated/fourtwentycoins.js/issues/370
    it('parses transaction function', function () {
        ...
        var tx = fourtwentycoins_1.fourtwentycoins.utils.parseTransaction(rawTx);
...
describe('Test Number Coder', function () {
    ...
    it('encodes zero', function () {
        var ZeroValues = [
            ...
            { n: 'BigNumber', v: fourtwentycoins_1.fourtwentycoins.constants.Zero }
        ];
    ...
    it('encodes one', function () {
        var ZeroValues = [
            ...
            { n: 'BigNumber', v: fourtwentycoins_1.fourtwentycoins.constants.One }
        ];
        var expected = oneHex;
        ['uint8', 'uint256', 'int8', 'int256'].forEach(function (type) {
            ZeroValues.forEach(function (value) {
    ...
    it('encodes negative one', function () {
        var Values = [
            ...
            { n: 'hex leading even length', v: '-0x0001' },
            { n: 'hex leading odd length', v: '-0x00001' },
            { n: 'BigNumber', v: fourtwentycoins_1.fourtwentycoins.constants.NegativeOne }
        ];
        var expected = maxHex;
    ...
    it('encodes full uint8 range', function () {
        for (var i = 0; i < 256; i++) {
            ...
            expected += fourtwentycoins_1.fourtwentycoins.utils.hexlify(i).substring(2);
    ...
    it('encodes full int8 range', function () {
        for (var i = -128; i < 128; i++) {
            ...
            if (i === -128) {
                ...
                expected += fourtwentycoins_1.fourtwentycoins.utils.hexlify(256 + i).substring(2);
            }
            else {
...
describe('Test Filters', function () {
    ...
    function doTest(test) {
        it(test.name, function () {
            var iface = new fourtwentycoins_1.fourtwentycoins.utils.Interface([test.signature]);
    ...
    var Tests = [
        ...
        // See: https://github.com/420integrated/fourtwentycoins.js/issues/305
        {
        ...
        // https://fourtwentyvm.io/tx/0x820cc57bc77be44d8f4f024a18e18f64a8b6e62a82a3d7897db5970dbe181ba1
        {
...
describe("Test ParamType Parser", function () {
    ...
    Tests.forEach(function (test) {
        it("allows correct modifiers " + JSON.stringify(test.type), function () {
            var paramType = fourtwentycoins_1.fourtwentycoins.utils.ParamType.from(test.type);
