...
const provider = new fourtwentycoins.providers.InfuraProvider("rinkeby", "49a0efa3aaee4fd99797bfa94d8ce2f1");
//const provider = fourtwentycoins.getDefaultProvider("rinkeby");

const TIMEOUT_PERIOD = 120000;

const contract = (function() {
    return new fourtwentycoins.Contract(contractData.contractAddress, contractData.interface, provider);
})();
...
function equals(name: string, actual: any, expected: any): void {
    ...
    if (typeof(actual) === 'object') {
        if (expected.indexed) {
            assert.ok(fourtwentycoins.Contract.isIndexed(actual), 'index property has index - ' + name);
            if (expected.hash) {
...
async function TestContractEvents() {
    const data = await fourtwentycoins.utils.fetchJson('https://api.fourtwentycoins.io/api/v1/?action=triggerTest&address=' + contract.address);
...
describe("Test Contract Transaction Population", function() {
    ...
    const contract = new fourtwentycoins.Contract(testAddress, abi);
    const contractConnected = contract.connect(fourtwentycoins.getDefaultProvider());
    ...
    it("allows send overrides", async function() {
        ...
        assert.ok(tx.smokeLimit.eq(150000), "smokeLimit matches");
        assert.ok(tx.smokePrice.eq(1900000000), "smokePrice matches");
...
/*
// Test Contract interaction inside Grid-deployed G420
