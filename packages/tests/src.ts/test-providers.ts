...
const bnify = fourtwentycoins.BigNumber.from;
    ...
    homestead: {
        addresses: [
            ...
            },
            {
                ...
                name: "ricmoo.firefly.fourtwenty"
            },
        ],
        blocks: [
            {
        ...
        transactions: [
            {
                ...
                smokePrice: bnify("0x4a817c800"),
                smokeLimit: bnify("0x3d090"),
    ...
    kovan: {
        ...
        blocks: [
            ...
            // Kovan Test Case with difficulty > 53-bits; See #711
            {
                ...
                difficulty: null,
                smokeLimit: bnify("0x989680"),
                smokeUsed: bnify("0x0705bf"),
        ...
        transactionReceipts: [
            {
                ...
                contractAddress: null,
                cumulativeSmokeUsed: bnify("0x80b9"),
                from: "0xb346D5019EeafC028CfC01A5f789399C2314ae8D",
                smokeUsed: bnify("0x80b9"),
                logs: [
                    {
            ...
            // Byzantium Receipt
            {
                ...
                contractAddress: null,
                cumulativeSmokeUsed: bnify("0x4142f"),
                from: "0xdc8F20170C0946ACCF9627b3EB1513CFD1c0499f",
                smokeUsed: bnify("0x1eb6d"),
                logs: [
                    {
    ...
    goerli: {
        ...
        blocks: [
            ...
            // Blockhash with leading zero; see #629
            {
                ...
                difficulty: 2,
                smokeLimit: bnify(0x7a1200),
                smokeUsed: bnify("0x0d0ef5"),
                miner: "0x0000000000000000000000000000000000000000",
        ...
        transactionReceipts: [
            {
                ...
                byzantium: true,
                contractAddress: null,
                cumulativeSmokeUsed: bnify(21000),
                from: "0x8c1e1e5b47980D214965f3bd8ea34C413E120ae4",
                smokeUsed: bnify(21000),
...
function equals(name: string, actual: any, expected: any): void {
    if (expected && expected.eq) {
        ...
        expected = fourtwentycoins.BigNumber.from(expected);
        actual = fourtwentycoins.BigNumber.from(actual);
...
type ProviderDescription = {
    ...
    create: (network: string) => fourtwentycoins.providers.Provider;
};
...
type TestDescription = {
    name: string;
    networks: Array<string>;
    execute: (provider: fourtwentycoins.providers.Provider) => Promise<void>;
...
const ApiKeys: Record<string, string> = {
    ...
    fourtwentyvm: "FPFGK6JSW2UHJJ2666FG93KP7WC999MNW7",
...
const providerFunctions: Array<ProviderDescription> = [
    {
        ...
        create: (network: string) => {
            if (network == "default") {
                return fourtwentycoins.getDefaultProvider(null, ApiKeys);
            }
            return fourtwentycoins.getDefaultProvider(network, ApiKeys);
        }
    },
    {
        ...
        create: (network: string) => {
            if (network == "default") {
                return new fourtwentycoins.providers.AlchemyProvider(null, ApiKeys.alchemy);
            }
            return new fourtwentycoins.providers.AlchemyProvider(network, ApiKeys.alchemy);
        }
    },
    /*
    {
        ...
        create: (network: string) => {
            return new fourtwentycoins.providers.CloudflareProvider(network);
        }
    },
    */
    {
        ...
        create: (network: string) => {
            if (network == "default") {
                return new fourtwentycoins.providers.InfuraProvider(null, ApiKeys.infura);
            }
            return new fourtwentycoins.providers.InfuraProvider(network, ApiKeys.infura);
        }
    },
    {
        name: "FourtwentyvmProvider",
        networks: allNetworks,
        create: (network: string) => {
            if (network == "default") {
                return new fourtwentycoins.providers.FourtwentyvmProvider(null, ApiKeys.fourtwentyvm);
            }
            return new fourtwentycoins.providers.FourtwentyvmProvider(network, ApiKeys.fourtwentyvm);
        }
    },
    {
    ...
    },
    {
        ...
        create: (network: string) => {
            if (network == "default") {
                return new fourtwentycoins.providers.PocketProvider(null, ApiKeys.pocket);
            }
            return new fourtwentycoins.providers.PocketProvider(network, ApiKeys.pocket);
        }
    },
    {
        name: "Web3Provider",
...
const fundWallet = fourtwentycoins.Wallet.createRandom();


const testFunctions: Array<TestDescription> = [ ];

Object.keys(blockchainData).forEach((network) => {
    function addSimpleTest(name: string, func: (provider: fourtwentycoins.providers.Provider) => Promise<any>, expected: any) {
        testFunctions.push({
            name: name,
            networks: [ network ],
            execute: async (provider: fourtwentycoins.providers.Provider) => {
    ...
    function addObjectTest(name: string, func: (provider: fourtwentycoins.providers.Provider) => Promise<any>, expected: any, checkSkip?: CheckSkipFunc) {
        testFunctions.push({
            name,
            networks: [ network ],
            checkSkip,
    ...
    tests.addresses.forEach((test) => {
        if (test.balance) {
            addSimpleTest(`fetches account balance: ${ test.address }`, (provider: fourtwentycoins.providers.Provider) => {
                return provider.getBalance(test.address);
            }, test.balance);
        }

        if (test.code) {
            addSimpleTest(`fetches account code: ${ test.address }`, (provider: fourtwentycoins.providers.Provider) => {
        ...
        if (test.storage) {
            Object.keys(test.storage).forEach((position) => {
                addSimpleTest(`fetches storage: ${ test.address }:${ position }`, (provider: fourtwentycoins.providers.Provider) => {
        ...
        if (test.name) {
            addSimpleTest(`fetches ENS name: ${ test.address }`, (provider: fourtwentycoins.providers.Provider) => {
    ...
    tests.blocks.forEach((test) => {
        addObjectTest(`fetches block (by number) #${ test.number }`, (provider: fourtwentycoins.providers.Provider) => {
            return provider.getBlock(test.number);
        }, test);
    });

    tests.blocks.forEach((test) => {
        addObjectTest(`fetches block (by hash) ${ test.hash }`, (provider: fourtwentycoins.providers.Provider) => {
            return provider.getBlock(test.hash);
        }, test, (provider: string, network: string, test: TestDescription) => {
            return (provider === "FourtwentyvmProvider");
        });
    });

    tests.transactions.forEach((test) => {
        addObjectTest(`fetches transaction ${ test.hash }`, async (provider: fourtwentycoins.providers.Provider) => {
    ...
    tests.transactionReceipts.forEach((test) => {
        addObjectTest(`fetches transaction receipt ${ test.transactionHash }`, async (provider: fourtwentycoins.providers.Provider) => {
...
(function() {
    function addErrorTest(code: string, func: (provider: fourtwentycoins.providers.Provider) => Promise<any>) {
        testFunctions.push({
    ...
    addErrorTest(fourtwentycoins.utils.Logger.errors.NONCE_EXPIRED, async (provider: fourtwentycoins.providers.Provider) => {
    ...
    addErrorTest(fourtwentycoins.utils.Logger.errors.INSUFFICIENT_FUNDS, async (provider: fourtwentycoins.providers.Provider) => {

        const txProps = {
            to: "0x8ba1f109551bD432803012645Ac136ddd64DBA72",
            smokePrice: 9000000000,
            smokeLimit: 21000,
            value: 1
        };

        const wallet = fourtwentycoins.Wallet.createRandom();
    ...
    addErrorTest(fourtwentycoins.utils.Logger.errors.INSUFFICIENT_FUNDS, async (provider: fourtwentycoins.providers.Provider) => {
        ...
        const wallet = fourtwentycoins.Wallet.createRandom().connect(provider);
        return wallet.sendTransaction(txProps);
    });

    addErrorTest(fourtwentycoins.utils.Logger.errors.UNPREDICTABLE_SMOKE_LIMIT, async (provider: fourtwentycoins.providers.Provider) => {
        return provider.estimateSmoke({
...
testFunctions.push({
    ...
    execute: async (provider: fourtwentycoins.providers.Provider) => {
        ...
        assert.ok(b0.gt(fourtwentycoins.constants.Zero), "balance is non-zero");
...
describe("Test Provider Methods", function() {
    let fundReceipt: Promise<fourtwentycoins.providers.TransactionReceipt> = null;
    const faucet = "0x8210357f377E901f18E45294e86a2A32215Cc3C9";

    before(async function() {
        this.timeout(300000);

        // Get some 420coin from the faucet
        const provider = new fourtwentycoins.providers.InfuraProvider("ropsten", ApiKeys.infura);
        const funder = await fourtwentycoins.utils.fetchJson(`https:/\/api.fourtwentycoins.io/api/v1/?action=fundAccount&address=${ fundWallet.address.toLowerCase() }`);
    ...
    after(async function() {
        ...
        // Refund all unused 420coin to the faucet
        const provider = new fourtwentycoins.providers.InfuraProvider("ropsten", ApiKeys.infura);
        const smokePrice = await provider.getSmokePrice();
        ...
        const tx = await fundWallet.connect(provider).sendTransaction({
            ...
            value: balance.sub(smokePrice.mul(21000))
        });
...
describe("Extra tests", function() {
    it("fourtwentyvm long-request #1093", async function() {
        this.timeout(60000);
        await waiter(2000);
        const provider = new fourtwentycoins.providers.FourtwentyvmProvider(null, ApiKeys.fourtwentyvm);
...
/*
describe("Test extra Fourtwentyvm operations", function() {
    let provider = new providers.FourtwentyvmProvider();
    it("fethces the current price of fourtwentycoin", function() {
        this.timeout(20000);
        return provider.getFourtwentycoinPrice().then(function(price) {
            assert.ok(typeof(price) === "number", "Fourtwentyvm price returns a number");
            assert.ok(price > 0.0, "Fourtwentyvm price returns non-zero");
        });
    });
    it("fetches the history", function() {
        this.timeout(100000);
        return provider.getHistory("ricmoo.firefly.fourtwenty").then(function(history) {
            assert.ok(history.length > 40, "Fourtwentyvm history returns results");
            assert.equal(history[0].hash, "0xd25f550cfdff90c086a6496a84dbb2c4577df15b1416e5b3319a3e4ebb5b25d8", "Fourtwentyvm history returns correct transaction");
        });
    });
});
*/
...
describe("Test Basic Authentication", function() {
    ...
    it("tests insecure connections fail", function() {
        this.timeout(60000);
        assert.throws(() => {
            return fourtwentycoins.utils.fetchJson(insecure);
        }, (error: Error) => {
...
describe("Test API Key Formatting", function() {
    it("Infura API Key", function() {
        ...
        const apiKeyString = fourtwentycoins.providers.InfuraProvider.getApiKey(projectId);
        ...
        const apiKeyObject = fourtwentycoins.providers.InfuraProvider.getApiKey({
            projectId
        });
        ...
        const apiKeyObject2 = fourtwentycoins.providers.InfuraProvider.getApiKey({
            projectId: projectId,
        ...
        assert.throws(() => {
            const apiKey = fourtwentycoins.providers.InfuraProvider.getApiKey({
                projectId: 1234,
                projectSecret: projectSecret
            });
            console.log(apiKey);
        ...
        {
            const provider = new fourtwentycoins.providers.InfuraProvider("homestead", {
                projectId: projectId,
                projectSecret: projectSecret
            });
        ...
        assert.throws(() => {
            const provider = new fourtwentycoins.providers.InfuraProvider("imaginary");
    ...
    it("Pocket API key", function() {
        ...
        const apiKeyString = fourtwentycoins.providers.PocketProvider.getApiKey(applicationId);
        ...
        const apiKeyObject = fourtwentycoins.providers.PocketProvider.getApiKey({
            applicationId
        });
        ...
        const apiKeyObject2 = fourtwentycoins.providers.PocketProvider.getApiKey({
        ...
        assert.throws(() => {
            const apiKey = fourtwentycoins.providers.PocketProvider.getApiKey({
                applicationId: 1234,
                applicationSecretKey: applicationSecretKey
            });
            console.log(apiKey);
        ...
        {
            const provider = new fourtwentycoins.providers.PocketProvider("homestead", {
                applicationId: applicationId,
                applicationSecretKey: applicationSecretKey
            });
            assert.equal(provider.network.name, "homestead");
        ...
        // Attempt an unsupported network
        assert.throws(() => {
            const provider = new fourtwentycoins.providers.PocketProvider("imaginary");
            console.log(provider);
        }, (error: any) => {
            return (error.argument === "network" && error.reason === "unsupported network");
        });
...
describe("Test WebSocketProvider", function() {
    ...
    async function testWebSocketProvider(provider: fourtwentycoins.providers.WebSocketProvider): Promise<void> {
        await provider.destroy();
    }

    it("InfuraProvider.getWebSocketProvider", async function() {
        const provider = fourtwentycoins.providers.InfuraProvider.getWebSocketProvider();
...
describe("Test Events", function() {
    ...
    async function testBlockEvent(provider: fourtwentycoins.providers.Provider) {
    ...
    it("InfuraProvider", async function() {
        this.timeout(60000);
        const provider = new fourtwentycoins.providers.InfuraProvider("rinkeby");
