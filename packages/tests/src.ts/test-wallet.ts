...
describe('Test JSON Wallets', function() {
    ...
    tests.forEach(function(test) {
        it(('decrypts wallet - ' + test.name), async function() {
            ...
            if (test.hasAddress) {
                assert.ok((fourtwentycoins.utils.getJsonWalletAddress(test.json) !== null),
                    'detect encrypted JSON wallet');
            }

            const wallet = await fourtwentycoins.Wallet.fromEncryptedJson(test.json, test.password);
            ...
            // Test connect
            {
                const provider = new fourtwentycoins.providers.FourtwentyvmProvider();
            ...
            {
                const wallet2 = new fourtwentycoins.Wallet(wallet._signingKey());
            ...
            if (test.name === "life") {
                const wallet2 = fourtwentycoins.Wallet.fromEncryptedJsonSync(test.json, test.password);
                assert.equal(wallet2.privateKey, test.privateKey,
                    'generated correct private key - ' + wallet2.privateKey);
            }
    ...
    ['one', 'two', 'three'].forEach(function(i) {
        ...
        let wallet = fourtwentycoins.Wallet.createRandom({ path: "m/56'/82", extraEntropy: utils.randomHexString('test-' + i, 32) });

        it('encrypts and decrypts a random wallet - ' + i, function() {
            this.timeout(1200000);

            return wallet.encrypt(password).then((json: string) => {
                return fourtwentycoins.Wallet.fromEncryptedJson(json, password).then((decryptedWallet) => {
...
describe('Test Transaction Signing and Parsing', function() {
    function checkTransaction(parsedTransaction: any, test: TestCase.SignedTransaction): any {
        ...
        ['nonce', 'smokeLimit', 'smokePrice', 'to', 'value', 'data'].forEach((key: (keyof TestCase.SignedTransaction)) => {
            let expected = test[key];

            let value = parsedTransaction[key];

            if ([ "smokeLimit", "smokePrice", "value"].indexOf(key) >= 0) {
                assert.ok((fourtwentycoins.BigNumber.isBigNumber(value)),
                ...
                value = fourtwentycoins.utils.hexlify(value);
                ...
                if (value) {
                    ...
                    fourtwentycoins.utils.getAddress(value);
    ...
    tests.forEach((test) => {
        it(('parses and signs transaction - ' + test.name), function() {
            ...
            let signingKey = new fourtwentycoins.utils.SigningKey(test.privateKey);
            let signDigest = signingKey.signDigest.bind(signingKey);

            // Legacy parsing unsigned transaction
            checkTransaction(fourtwentycoins.utils.parseTransaction(test.unsignedTransaction), test);

            let parsedTransaction = fourtwentycoins.utils.parseTransaction(test.signedTransaction);
            let transaction = checkTransaction(parsedTransaction, test);

            // Legacy signed transaction ecrecover
            assert.equal(parsedTransaction.from, fourtwentycoins.utils.getAddress(test.accountAddress),
                'computed from');
            ...
            (function() {
                let unsignedTx = fourtwentycoins.utils.serializeTransaction(transaction);
                ...
                let signature = signDigest(fourtwentycoins.utils.keccak256(unsignedTx));
                assert.equal(fourtwentycoins.utils.serializeTransaction(transaction, signature), test.signedTransaction,
            ...
            let parsedUnsignedTransactionChainId5 = fourtwentycoins.utils.parseTransaction(test.unsignedTransactionChainId5);
            ...
            // EIP-155 fields
            let parsedTransactionChainId5 = fourtwentycoins.utils.parseTransaction(test.signedTransactionChainId5);
            ...
            type TxNumberKey = 'smokeLimit' | 'smokePrice' | 'value';
            ['smokeLimit', 'smokePrice', 'value'].forEach((key: TxNumberKey) => {
            ...
            (function() {
                ...
                let signature = signDigest(fourtwentycoins.utils.keccak256(unsignedTx));
                assert.equal(fourtwentycoins.utils.serializeTransaction(transaction, signature), test.signedTransactionChainId5,
...
describe('Test Signing Messages', function() {
    ...
    let tests: Array<TestCase> = [
        ...
        // See: https://github.com/420integrated/fourtwentycoins.js/issues/80
        {
            ...
            message: fourtwentycoins.utils.arrayify('0x47173285a8d7341e5e972fc677286384f802f8ef42a5ec5f03bbfa254cb01fad'),
        ...
        // See: https://github.com/420integrated/fourtwentycoins.js/issues/85
        {
            ...
            message: fourtwentycoins.utils.arrayify(fourtwentycoins.utils.id('0x7f23b5eed5bc7e89f267f339561b2697faab234a2')),
    ...
    tests.forEach(function(test) {
        it(('verifies a message "' + test.name + '"'), function() {
            this.timeout(120000);
            let address = fourtwentycoins.utils.verifyMessage(test.message, test.signature);
    ...
    tests.forEach(function(test) {
      it(('hashes a message "' + test.name + '"'), function() {
          this.timeout(120000);
          let hash = fourtwentycoins.utils.hashMessage(test.message);
...
describe("Serialize Transactions", function() {
    it("allows odd-length numeric values", function() {
        fourtwentycoins.utils.serializeTransaction({
            smokeLimit: "0x1",
            smokePrice: "0x1",
            value: "0x1"
        });
...
describe("Wallet Errors", function() {
    it("fails on privateKey/address mismatch", function() {
        assert.throws(() => {
            const wallet = new fourtwentycoins.Wallet({
    ...
    it("fails on mnemonic/address mismatch", function() {
        assert.throws(() => {
            const wallet = new fourtwentycoins.Wallet(<any>{
                privateKey: "0x6a73cd9b03647e83ef937888a5258a26e4c766dbf41ddd974f15e32d09cfe9c0",
    ...
    it("fails on from mismatch", function() {
        const wallet = new fourtwentycoins.Wallet("0x6a73cd9b03647e83ef937888a5258a26e4c766dbf41ddd974f15e32d09cfe9c0");
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (error.code === fourtwentycoins.utils.Logger.errors.INVALID_ARGUMENT && error.argument === "transaction.from") {
                    resolve(true);
                    return;
                }
            }
