...
function equals(actual: any, expected: any): boolean {
    ...
    // Uint8Array
    if (expected.buffer) {
        if (!fourtwentycoins.utils.isHexString(actual)) { return false; }
        actual = fourtwentycoins.utils.arrayify(actual);
    ...
    // Maybe address?
    try {
        let actualAddress = fourtwentycoins.utils.getAddress(actual);
        let expectedAddress = fourtwentycoins.utils.getAddress(expected);
...
function getValues(object: any, named?: boolean): any {
   ...
   switch (object.type) {
       ...
       case 'buffer':
           return fourtwentycoins.utils.arrayify(object.value);
...
describe('ABI Coder Encoding', function() {
    let coder = fourtwentycoins.utils.defaultAbiCoder;
...
describe('Test Contract Events', function() {
    ...
    tests.forEach((test, index) => {
        it(('decodes event parameters - ' + test.name + ' - ' + test.types), function() {
            ...
            let iface = new fourtwentycoins.utils.Interface(test.interface);
        ...
        it(('decodes event data - ' + test.name + ' - ' + test.types), function() {
            ...
            test.normalizedValues.forEach((expected, index) => {
                if (test.indexed[index]) {
                    assert.ok((fourtwentycoins.Contract.isIndexed(parsed[index]) && parsed[index].hash == null), 'parsed event data has empty Indexed - ' + index);
                } else {
...
describe('Test Interface Signatures', function() {
    ...
    tests.forEach((test) => {
        it('derives the correct signature - ' + test.name, function() {
            let iface = new fourtwentycoins.utils.Interface(test.abi);
            this.timeout(120000);
    ...
    it('derives correct description for human-readable ABI', function() {
        let iface = new fourtwentycoins.utils.Interface([ "function transfer(address from, uint amount)" ]);
        [
            "transfer",
    ...
    it ('parses transaction function', function() {
        ...
        let tx = fourtwentycoins.utils.parseTransaction(rawTx);
...
describe('Test Number Coder', function() {
    ...
    it('encodes zero', function() {
        let ZeroValues = [
            ...
            { n: 'BigNumber', v: fourtwentycoins.constants.Zero }
        ];
    ...
    it('encodes one', function() {
        let ZeroValues = [
            ...
            { n: 'BigNumber', v: fourtwentycoins.constants.One }
        ];
    ...
    it('encodes negative one', function() {
        let Values = [
            ...
            { n: 'hex leading even length', v: '-0x0001' },
            { n: 'hex leading odd length', v: '-0x00001' },
            { n: 'BigNumber', v: fourtwentycoins.constants.NegativeOne }
        ];
    ...
    it('encodes full uint8 range', function() {
        for (let i = 0; i < 256; i++) {
            ...
            expected += fourtwentycoins.utils.hexlify(i).substring(2);
    ...
    it('encodes full int8 range', function() {
        for (let i = -128; i < 128; i++) {
            ...
            if (i === -128) {
                ...
                expected += fourtwentycoins.utils.hexlify(256 + i).substring(2);
            } else {
...
describe('Test Filters', function() {
    ...
    function doTest(test: TestCase) {
        it(test.name, function() {
            let iface = new fourtwentycoins.utils.Interface([ test.signature ]);
...
describe("Test ParamType Parser", function() {
    ...
    Tests.forEach((test) => {
        it(`allows correct modifiers ${ JSON.stringify(test.type) }`, function() {
            const paramType = fourtwentycoins.utils.ParamType.from(test.type);
