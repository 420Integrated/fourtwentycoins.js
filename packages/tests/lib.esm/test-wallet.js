    ...
    tests.forEach(function (test) {
        it(('decrypts wallet - ' + test.name), function () {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                if (test.hasAddress) {
                    assert.ok((fourtwentycoins.utils.getJsonWalletAddress(test.json) !== null), 'detect encrypted JSON wallet');
                }
                const wallet = yield fourtwentycoins.Wallet.fromEncryptedJson(test.json, test.password);
...
describe('Test Transaction Signing and Parsing', function () {
    function checkTransaction(parsedTransaction, test) {
        let transaction = {};
        ['nonce', 'smokeLimit', 'smokePrice', 'to', 'value', 'data'].forEach((key) => {
            let expected = test[key];
            let value = parsedTransaction[key];
            if (["smokeLimit", "smokePrice", "value"].indexOf(key) >= 0) {
                assert.ok((fourtwentycoins.BigNumber.isBigNumber(value)), 'parsed into a big number - ' + key);
    ...
    tests.forEach((test) => {
        it(('parses and signs transaction - ' + test.name), function () {
            ...
            assert.equal(parsedTransaction.from, fourtwentycoins.utils.getAddress(test.accountAddress), 'computed from');
            ...
            (function () {
                ...
                assert.equal(fourtwentycoins.utils.serializeTransaction(transaction, signature), test.signedTransaction, 'signs transaction (legacy)');
            })();
            // EIP155
            ...
            (function () {
                ...
                let signature = signDigest(fourtwentycoins.utils.keccak256(unsignedTx));
                assert.equal(fourtwentycoins.utils.serializeTransaction(transaction, signature), test.signedTransactionChainId5, 'signs transaction (eip155)');
            })();
        });
    });
    tests.forEach((test) => {
