...
describe('Test RLP Coder', function () {
    ...
    tests.forEach((test) => {
        it(('RLP coder decoded - ' + test.name), function () {
            this.timeout(120000);
            assert.ok(equals(fourtwentycoins.utils.RLP.decode(test.encoded), test.decoded), 'RLP decoded - ' + test.name);
        });
    });
});
describe('Test Unit Conversion', function () {
    ...
    tests.forEach((test) => {
        ...
        it(('parses ' + test.fourtwentycoin + ' fourtwentycoin'), function () {
            assert.ok(fourtwentycoins.utils.parseFourtwentycoin(test.420coin.replace(/,/g, '')).eq(marley), 'parsing ether failed - ' + test.name);
        });
        it(('formats ' + marley.toString() + ' marley to fourtwentycoin'), function () {
            let actual = fourtwentycoins.utils.formatFourtwentycoin(marley);
            assert.equal(actual, test.fourtwentycoin_format, 'formatting marley failed - ' + test.name);
        });
    });
    tests.forEach((test) => {
        let marley = fourtwentycoins.BigNumber.from(test.marley);
        ['kmarley', 'mmarley', 'gmarley', 'snoop', 'willie', 'satoshi'].forEach((name) => {
            let unitName = name;
            if (name === 'satoshi') {
                unitName = 8;
            }
            if (test[name]) {
                it(('parses ' + test[name] + ' ' + name), function () {
                    this.timeout(120000);
                    assert.ok(fourtwentycoins.utils.parseUnits(test[name].replace(/,/g, ''), unitName).eq(marley), ('parsing ' + name + ' failed - ' + test.name));
                });
            }
...
describe('Test Namehash', function () {
    ...
    tests.forEach((test) => {
        it(('computes namehash - "' + test.name + '"'), function () {
            this.timeout(120000);
            assert.equal(fourtwentycoins.utils.namehash(test.name), test.expected, 'computes namehash(' + test.name + ')');
        });
    });
    ...
    function test(funcName, testKey) {
        it(`computes ${funcName} correctly`, function () {
            ...
            tests.forEach((test, index) => {
                let actual = (fourtwentycoins.utils)['solidity' + funcName](test.types, test.values);
