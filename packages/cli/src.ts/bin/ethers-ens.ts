...
import { Base58 } from "@420integrated/basex";
...
const fourtwentyLegacyRegistrarAbi = [
...
abstract class EnsPlugin extends Plugin {
    _fourtwentyAddressCache: { [ addressOrInterfaceId: string ]: string };

    constructor() {
        super();
        fourtwentycoins.utils.defineReadOnly(this, "_fourtwentyAddressCache", { });
    }

    getEns(): fourtwentycoins.Contract {
        return new fourtwentycoins.Contract(this.network.ensAddress, ensAbi, this.accounts[0] || this.provider);
    }

    async getResolver(nodehash: string): Promise<fourtwentycoins.Contract> {
        if (!this._fourtwentyAddressCache[nodehash]) {
            this._fourtwentyAddressCache[nodehash] = await this.getEns().resolver(nodehash);
        }
        return new fourtwentycoins.Contract(this._fourtwentyAddressCache[nodehash], resolverAbi, this.accounts[0] || this.provider);
    }

    async getFourtwentyInterfaceAddress(interfaceId: string): Promise<string> {
        let fourtwentyNodehash = fourtwentycoins.utils.namehash("fourtwenty");
        if (!this._fourtwentyAddressCache[interfaceId]) {
            let resolver = await this.getResolver(fourtwentyNodehash);
            this._fourtwentyAddressCache[interfaceId] = await resolver.interfaceImplementer(fourtwentyNodehash, interfaceId);
        }
        return this._fourtwentyAddressCache[interfaceId];
    }

    async getFourtwentyController(): Promise<fourtwentycoins.Contract> {
        let address = await this.getFourtwentyInterfaceAddress(InterfaceID_Controller);
        return new fourtwentycoins.Contract(address, fourtwentyControllerAbi, this.accounts[0] || this.provider);
    }

    async getFourtwentyLegacyRegistrar(): Promise<fourtwentycoins.Contract> {
        let address = await this.getFourtwentyInterfaceAddress(InterfaceID_Legacy);
        return new fourtwentycoins.Contract(address, fourtwentyLegacyRegistrarAbi, this.accounts[0] || this.provider);
    }

    async getFourtwentyRegistrar(): Promise<fourtwentycoins.Contract> {
        //let address = await this.getFourtwentyInterfaceAddress(InterfaceID_ERC721);
        let address = await this.getEns().owner(fourtwentycoins.utils.namehash("fourtwenty"));
        return new fourtwentycoins.Contract(address, fourtwentyRegistrarAbi, this.accounts[0] || this.provider);
    }
}
...
class LookupPlugin extends EnsPlugin {
    ...
    async run(): Promise<void> {
        ...
        let controller = await this.getFourtwentyController();
        let registrar = await this.getFourtwentyRegistrar();
        let legacyRegistrar = await this.getFourtwentyLegacyRegistrar();

        for (let i = 0; i < this.names.length; i++) {
            let name = this.names[i];

            let nodehash = fourtwentycoins.utils.namehash(name);
            ...
            if (owner === fourtwentycoins.constants.AddressZero) {
                owner = null;
            } else {
                details.Controller = owner;
                details.Resolver = await ens.resolver(nodehash).then((address: string) => {
                    if (address === fourtwentycoins.constants.AddressZero) {
            ...
            if (comps.length === 2 && comps[1] === "fourtwenty") {
                details.Labelhash = fourtwentycoins.utils.id(comps[0].toLowerCase()); // @TODO: nameprep
                ...
                if (!details.Available) {
                    try {
                        ...
                        if (ownerOf !== fourtwentycoins.constants.AddressZero) {
                            details.Registrant = ownerOf;
                            details.Registrar = "Permanent";
                        }
                    } catch (error) {
                        let entry = await legacyRegistrar.entries(details.Labelhash);
                        let deed = new fourtwentycoins.Contract(entry.owner, deedAbi, this.provider);

                        details.Registrant = await deed.owner();
                        details.Registrar = "Legacy";
                        details["Deed Value"] = (fourtwentycoins.utils.formatFourtwentycoin(entry.value) + " fourtwentycoin");
                        details["Highest Bid"] = (fourtwentycoins.utils.formatFourtwentycoin(entry.highestBid) + " fourtwentycoin");
                    }
                }
            }

            if (resolverAddress) {
                let resolver = new fourtwentycoins.Contract(resolverAddress, resolverAbi, this.provider);
                ...
                let content = await resolver.contenthash(nodehash).then((hash: string) => {
                    if (hash === "0x") { return ""; }
                    if (hash.substring(0, 10) === "0xe3010170" && fourtwentycoins.utils.isHexString(hash, 38)) {
                        return Base58.encode(fourtwentycoins.utils.hexDataSlice(hash, 4)) + " (IPFS)";
                    }
...
abstract class AccountPlugin extends EnsPlugin {
    ...
    static getHelp(): Help {
        return logger.throwError("subclasses must implement this", fourtwentycoins.errors.UNSUPPORTED_OPERATION, {
            operation: "getHelp"
        });
    }

    async _setValue(key: string, value: string): Promise<void> {
        fourtwentycoins.utils.defineReadOnly<any, any>(this, key, value);
        if (key === "name") {
            await this._setValue("nodehash", fourtwentycoins.utils.namehash(value));
        }
    }
    ...
    async prepareArgs(args: Array<string>): Promise<void> {
        ...
        let helpLine = fourtwentycoins.utils.getStatic<() => Help>(this.constructor, "getHelp")().name;
...
abstract class ControllerPlugin extends AccountPlugin {
    ...
    async prepareOptions(argParser: ArgParser): Promise<void> {
        ...
        if (secret) {
            ...
            this.salt = fourtwentycoins.utils.id(secret);
        }
        ...
        fourtwentycoins.utils.defineReadOnly(this, "duration", duration * (60 * 60 * 24));
    }
    ...
    async prepareArgs(args: Array<string>): Promise<void> {
        ...
        if (!this.salt) {
            ...
            this.salt = fourtwentycoins.utils.keccak256(signature);
        }
    }
}
...
class CommitPlugin extends ControllerPlugin {
    ...
    async run(): Promise<void> {
        await super.run();
        let fourtwentyController = await this.getFourtwentyController();

        let commitment = await fourtwentyController.makeCommitment(this.label, this.owner, this.salt);
        let fee = await fourtwentyController.rentPrice(this.label, this.duration);
        ...
        this.dump("Commit: " + this.name, {
            ...
            Fee: fourtwentycoins.utils.formatFourtwentycoin(fee) + " (informational)",
            Commitment: commitment
        });

        await fourtwentyController.commit(commitment);
    }
}
...
class RevealPlugin extends ControllerPlugin {
    ...
    async run(): Promise<void> {
        ...
        this.dump("Reveal: " + this.name, {
            ...
            Fee: fourtwentycoins.utils.formatFourtwentycoin(fee),
        });

        await fourtwentyController.register(this.label, this.owner, this.duration, this.salt, {
...
class SetSubnodePlugin extends AddressAccountPlugin {
    ...
    async run(): Promise<void> {
        ...
        await this.getEns().setSubnodeOwner(fourtwentycoins.utils.namehash(this.node), fourtwentycoins.utils.id(this.label), this.address);
    }
}
...
class SetNamePlugin extends AddressAccountPlugin {
    ...
    async run(): Promise<void> {
        ...
        const nodehash = fourtwentycoins.utils.namehash(this.address.substring(2) + ".addr.reverse");
...
class SetContentPlugin extends AccountPlugin {
    ...
    async _setValue(key: string, value: string): Promise<void> {
        if (key === "hash") {
            ...
            let multihash = fourtwentycoins.utils.concat([ "0xe3010170", bytes ]);
            await super._setValue("multihash", fourtwentycoins.utils.hexlify(multihash));
        }
        await super._setValue(key, value);
    }
...
class MigrateRegistrarPlugin extends AccountPlugin {
    ...
    readonly deedValue: fourtwentycoins.BigNumber;
    readonly highestBid: fourtwentycoins.BigNumber;
    ...
    async prepareArgs(args: Array<string>): Promise<void> {
        ...
        let fourtwentyLegacyRegistrar = await this.getFourtwentyLegacyRegistrar();
        let entry: any = await fourtwentyLegacyRegistrar.entries(fourtwentycoins.utils.id(comps[0]));
    ...
    async run(): Promise<void> {
        ...
        this.dump("Migrate Registrar: " + this.name, {
            "Nodehash": this.nodehash,
            "Highest Bid": (fourtwentycoins.utils.formatFourtwentycoin(this.highestBid) + " fourtwentycoin"),
            "Deed Value": (fourtwentycoins.utils.formatFourtwentycoin(this.deedValue) + " fourtwentycoin"),
        });

        let legacyRegistrar = await this.getFourtwentyLegacyRegistrar();
        await legacyRegistrar.transferRegistrars(fourtwentycoins.utils.id(this.label));
    }
}
...
class TransferPlugin extends AccountPlugin {
    ...
    async run(): Promise<void> {
        ...
        let registrar = await this.getFourtwentyRegistrar();
        await registrar.safeTransferFrom(this.accounts[0].getAddress(), this.new_owner, fourtwentycoins.utils.id(this.label));
    }
}
...
class ReclaimPlugin extends AddressAccountPlugin {
    ...
    async _setValue(key: string, value: string): Promise<void> {
        if (key === "name") {
            ...
            try {
                ownerOf = await registrar.ownerOf(fourtwentycoins.utils.id(comps[0]));
            } catch (error) {
    ...
    async run(): Promise<void> {
        ...
        let registrar = await this.getFourtwentyRegistrar();
        await registrar.reclaim(fourtwentycoins.utils.id(this.label), this.address);
    }
}
...
class RenewPlugin extends EnsPlugin {
    ...
    async run(): Promise<void> {
        ...
        const fourtwentyController = await this.getFourtwentyController();
        const fourtwentyRegistrar = await this.getFourtwentyRegistrar();

        for (let i = 0; i < this.labels.length; i++) {
            const label = this.labels[i];
            console.log(label);

            const expiration = (await fourtwentyRegistrar.nameExpires(fourtwentycoins.utils.id(label))).toNumber();
            if (expiration === 0) {
            ...
            const fee = (await fourtwentyController.rentPrice(label, duration)).mul(11).div(10);
            ...
            this.dump(`Renew: ${ label }.fourtwenty`, {
                ...
                "Fee":            `${ fourtwentycoins.utils.formatFourtwentycoin(fee) } (+10% buffer)`,
            });

            await fourtwentyController.renew(label, duration, {
                value: fee
            });
        }
    }
