...
function getType(param: fourtwentycoins.utils.ParamType, flexible?: boolean): string {
    ...
    if (param.type.substring(0, 5) === "bytes") {
        if (flexible) {
            return "string | fourtwentycoins.utils.BytesLike";
        }
        return "string"
    }
    ...
    if (match) {
        if (flexible) {
            return "fourtwentycoins.BigNumberish";
        }
        if (parseInt(match[2]) < 53) { return 'number'; }
        return 'fourtwentycoins.BigNumber';
    }
...
export const header = "import { fourtwentycoins } from \"fourtwentycoins \";\n\n"

export function generate(contract: ContractCode, bytecode?: string): string {

    let lines = [ ];

    lines.push("export class " + contract.name + " extends fourtwentycoins.Contract {");
    lines.push("");
    lines.push("    constructor(addressOrName: string, providerOrSigner: fourtwentycoins.Signer | fourtwentycoins.providers.Provider) {");
    lines.push("        super(addressOrName, new.target.ABI(), providerOrSigner)");
    lines.push("    }");
    lines.push("");
    lines.push(`    connect(providerOrSigner: fourtwentycoins.Signer | fourtwentycoins.providers.Provider): ${contract.name} {`);
    ...
    for (let signature in contract.interface.functions) {
        ...
        let output = "Promise<fourtwentycoins.providers.TransactionResponse>";

        let overrides = "fourtwentycoins.CallOverrides";
        if (fragment.constant == false) {
            if (fragment.payable) {
                overrides = "fourtwentycoins.PayableOverrides";
            } else {
                overrides = "fourtwentycoins.Overrides";
            }
    ...
    lines.push("");
    lines.push("    static factory(signer?: fourtwentycoins.Signer): fourtwentycoins.ContractFactory {");
    lines.push("        return new fourtwentycoins.ContractFactory(" + contract.name + ".ABI(), " + contract.name + ".bytecode(), signer);");
    lines.push("    }");

    lines.push("");
    lines.push("    static bytecode(): string {");
    if (bytecode == null) {
        lines.push('        return fourtwentycoins.errors.throwError("no bytecode provided during generation", fourtwentycoins.errors.UNSUPPORTED_OPERATION, { operation: "contract.bytecode" });');
    } else {
    ...
    contract.interface.fragments.forEach((fragment) => {
        lines.push(`            "${fragment.format(fourtwentycoins.utils.FormatTypes.full)}",`);
    });
    lines.push("        ];");
    lines.push("    }");
    lines.push("}");
