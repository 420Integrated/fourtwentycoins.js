...
function populateOptions(options) {
    options = fourtwentycoins_1.fourtwentycoins.utils.shallowCopy(options || {});
...
function _compile(_solc, source, options) {
    ...
    for (var filename in output.contracts) {
        for (var name_1 in output.contracts[filename]) {
            ...
            result.push({
                name: name_1,
                interface: new fourtwentycoins_1.fourtwentycoins.utils.Interface(contract.abi),
...
// and for solc will fallback onto the version included in @420integrated/cli
