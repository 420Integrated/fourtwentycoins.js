...
import { Block, BlockWithTransactions, Provider } from "@420integrated/abstract-provider";
import { BigNumber } from "@420integrated/bignumber";
import { isHexString } from "@420integrated/bytes";
import { Network } from "@420integrated/networks";
import { deepCopy, defineReadOnly, shallowCopy } from "@420integrated/properties";
import { shuffled } from "@420integrated/random";
...
async function getRunner(config: RunningConfig, currentBlockNumber: number, method: string, params: { [ key: string]: any }): Promise<any> {
    ...
    switch (method) {
        ...
        case "getFourtwentycoinPrice":
            if ((<any>provider).getFourtwentycoinPrice) {
                return (<any>provider).getFourtwentycoinPrice();
            }
            break;
        case "getBalance":
        case "getTransactionCount":
