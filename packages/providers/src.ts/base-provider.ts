...
} from "@420integrated/abstract-provider";
import { Base58 } from "@420integrated/basex";
import { BigNumber, BigNumberish } from "@420integrated/bignumber";
import { arrayify, concat, hexConcat, hexDataLength, hexDataSlice, hexlify, hexValue, hexZeroPad, isHexString } from "@420integrated/bytes";
import { HashZero } from "@420integrated/constants";
import { namehash } from "@420integrated/hash";
import { getNetwork, Network, Networkish } from "@420integrated/networks";
import { Deferrable, defineReadOnly, getStatic, resolveProperties } from "@420integrated/properties";
import { Transaction } from "@420integrated/transactions";
import { sha256 } from "@420integrated/sha2";
import { toUtf8Bytes, toUtf8String } from "@420integrated/strings";
import { poll } from "@420integrated/web";
...
export class BaseProvider extends Provider implements EnsProvider {
    ...
    async getSmokePrice(): Promise<BigNumber> {
        await this.getNetwork();
        return BigNumber.from(await this.perform("getSmokePrice", { }));
    }
    ...
    async estimateSmoke(transaction: Deferrable<TransactionRequest>): Promise<BigNumber> {
        await this.getNetwork();
        const params = await resolveProperties({
            transaction: this._getTransactionRequest(transaction)
        });
        return BigNumber.from(await this.perform("estimateSmoke", params));
    }
    ...
    async getFourtwentycoinPrice(): Promise<number> {
        await this.getNetwork();
