...
#### *signer* . **getSmokePrice**( ) => *Promise< [BigNumber](/v5/api/utils/bignumber/) >*

Returns the current smoke price.
...
#### *signer* . **estimateSmoke**( transactionRequest ) => *Promise< [BigNumber](/v5/api/utils/bignumber/) >*
...
This should return a **copy** of the *transactionRequest*, with any properties needed by `call`, `estimateSmoke` and `populateTransaction` (which is used by sendTransaction). It should also throw an error if any unknown key is specified.
...
The default implementation calls `checkTransaction` and resolves to if it is an ENS name, adds `smokePrice`, `nonce`, `smokeLimit` and `chainId` based on the related operations on Signer.


Wallet
------

#### **new ***fourtwentycoins* . **Wallet**( privateKey [ , provider ] )

Create a new Wallet instance for *privateKey* and optionally connected to the *provider*.


#### *fourtwentycoins* . *Wallet* . **createRandom**( [ options = {} ] ) => *[Wallet](/v5/api/signer/#Wallet)*
...
#### *fourtwentycoins* . *Wallet* . **fromEncryptedJson**( json , password [ , progress ] ) => *Promise< [Wallet](/v5/api/signer/#Wallet) >*
...
#### *fourtwentycoins* . *Wallet* . **fromEncryptedJsonSync**( json , password ) => *[Wallet](/v5/api/signer/#Wallet)*
...
#### *fourtwentycoins* . *Wallet* . **fromMnemonic**( mnemonic [ , path , [ wordlist ] ] ) => *[Wallet](/v5/api/signer/#Wallet)*

Create an instance from a mnemonic phrase.

If path is not specified, the 420coin default path is used (i.e. `m/44'/60'/0'/0/0`).
...
tx = {
  ...
  value: utils.parseFourtwentycoin("1.0")
}
...
#### **new ***fourtwentycoins* . **VoidSigner**( address [ , provider ] ) => *[VoidSigner](/v5/api/signer/#VoidSigner)*
...
signer = new fourtwentycoins.VoidSigner(address, provider)
...
contract = new fourtwentycoins.Contract("dai.tokens.fourtwentycoins.fourtwenty", abi, signer)
...
contract.callStatic.transfer("donations.fourtwentycoins.fourtwenty", tokens)
// { Promise: true }

// This will fail since it is greater than the token balance
contract.callStatic.transfer("donations.fourtwentycoins.fourtwenty", tokens.add(1))
