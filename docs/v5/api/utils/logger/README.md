...
#### **new ***fourtwentycoins* . *utils* . **Logger**( version )
...
A sending account must have enough 420coins to pay for the value, the smoke limit (at the smoke price) as well as the intrinsic cost of data. The intrinsic cost of data is 4 smoke for each zero byte and 68 smoke for each non-zero byte.


#### *Logger* . *errors* . **NETWORK_ERROR**

An 420coin network validation error, such as an invalid chain ID.
...
When replacing a transaction, by using a nonce which has already been sent to the network, but which has not been mined yet the new transaction must specify a higher smoke price.

This error occurs when the smoke price is insufficient to *bribe* the transaction pool to prefer the new transaction over the old one. Generally, the new smoke price should be about 50% + 1 marley more, so if a smoke price of 10 gmarley was used, the replacement should be 15.000000001 gmarley.


#### *Logger* . *errors* . **UNPREDICTABLE_SMOKE_LIMIT**

When estimating the required amount of smoke for a transaction, a node is queried for its best guess.

If a node is unable (or unwilling) to predict the cost, this error occurs.

The best remedy for this situation is to specify a smoke limit in the transaction manually.
