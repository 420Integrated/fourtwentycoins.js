...
function generate() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const versions = Object.keys(lines.reduce((accum, line, index) => {
            const match = line.match(/^fourtwentycoins \/v([^ ]*)/);
            if (match) {
                if (firstLine == null) {
                    firstLine = index;
                }
        ...
        const version = local.getPackage("fourtwentycoins").version;
        ;
        const published = yield npm.getPackage("fourtwentycoins");
        ...
        const newTitle = `fourtwentycoins/v${version} (${utils_1.getDateTime(new Date())})`;
        ...
        changes.forEach((change) => {
            ...
            let commit = `[${change.commit.substring(0, 7)}](https://github.com/420integrated/fourtwentycoins.js/commit/${change.commit})`;
            let link = commit;
            if (linkMatch) {
                body = body.replace(/ *(\(.*#.*)\) */, "");
                link = linkMatch[2].replace(/#([0-9]+)/g, (all, issue) => {
                    return `[#${issue}](https://github.com/420integrated/fourtwentycoins.js/issues/${issue})`;
                }) + "; " + commit;
            }
...
function getLatestChange() {
    ...
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        const match = line.match(/fourtwentycoins\/([^\(]*)\(([^\)]*)\)/);
        if (match) {
            if (result) {
                break;
            }
