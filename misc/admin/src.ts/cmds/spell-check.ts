...
bitcoin fourtwenty willie gmarley kmarley mmarley satoshi snoop marley w420
...
commify config
contenthash ctr ctrl debug dd dklen eexist encseed eof fourtwentyaddr
fourtwentyseed fourtwentycoins eval exec filename func gz gzip hid http https hw iv
...
alchemyapi Cloudflare Fourtwentyvm INFURA IPFS MetaMask Nodesmith
Trezor 420integrated axic bitcoinjs browserify easyseed fourtwentyjs
...
(async function() {
    ...
    getAllStrings(resolve(__dirname, "../../../../packages")).forEach((file: Foo) => {
        ...
        file.values.forEach((entry) => {
            ...
            // Prolly a require
            if (value.match(/^@420integrated\/[a-z0-9-]+$/)) { return; }
